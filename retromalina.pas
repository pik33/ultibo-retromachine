...
var fh,filetype:integer;                // this needs cleaning...
    ...
    kbdclick:byte=0;
    ch,oldch:tkeyboardreport;
    activekey:byte=0;
    oldactivekey:byte=0;
    lastactivekey:byte=0;
    olactivekey:byte=0;
    rptcnt:byte=0;
...
threadsleep(1);
  repeat
  repeat threadsleep(1); a:= lpeek($3F007600) until (a and 2) <>0 ;
  if (a and 2)<>0 then
    begin
...
// constructor procedure: setup ram, load data from files
...
// in every vblank, get events from kbd/mouse/joys etc,
// set system vars and cursors
...
// cursor
i:=(lpeek($2060000) div 15) mod 2 ; // todo - replace constant with sys var
lpoke($2060070,(68 shl 16) +32*(lpeek($20600a0)and $FFFF0000)+64+16*(lpeek($20600a0) and $FFFF));
// cursor blink
if i=0 then lpoke($2060070,lpeek($2060070) or $1000)
else lpoke($2060070,lpeek($2060070) and $FFFF0FFF);
...
// keyboard
  ...
  if ch[7]<>255 then
    ...
    lastactivekey:=0;
    activekey:=0;
    if ch[0]>0 then m:=ch[0];
    for i:=2 to 7 do if (ch[i]>3) and (ch[i]<255) then lastactivekey:=i;
    if (lastactivekey>olactivekey) and (lastactivekey>0) then begin rptcnt:=0; activekey:=ch[lastactivekey]; {box(100,100,100,100,34); outtextxyz(100,100,inttostr(lastactivekey),40,2,2);} end
    else if (lastactivekey<olactivekey) then begin rptcnt:=0; activekey:=0; {box(100,100,100,100,120); outtextxyz(100,100,inttostr(lastactivekey),40,2,2);}end
    else if (lastactivekey=olactivekey) and (lastactivekey>0) and (oldactivekey<>ch[lastactivekey]) then begin rptcnt:=0; activekey:=ch[lastactivekey]; {box(100,100,100,100,184); outtextxyz(100,100,inttostr(lastactivekey),40,2,2);}end;
    if lastactivekey<2 then begin rptcnt:=0; activekey:=0; m:=0; end;
 //   if (activekey>2) and (oldactivekey>2) and (activekey<>oldactivekey) and (olactivekey<lastactivekey) then begin rptcnt:=1; olactivekey:=lastactivekey; end;
...
//  box(200,200,100,100,82);
//  outtextxyz(200,200,inttostr(rptcnt),90,2,2);
  ...
  sndcnt+=1; if sndcnt>2 then sndcnt:=2 ;
  if sndcnt=1 then kbdclick:=1;
...
if kbdclick>=5 then begin kbdclick:=0; for i:=0 to 4095 do slpoke($0205a000+4*i,0); end;
if (kbdclick>0) and (kbdclick<5) then begin kbdclick+=1; for i:=0 to 191 do slpoke(b+4*i, 30000-60000*((i div 48) mod 2)); end;
